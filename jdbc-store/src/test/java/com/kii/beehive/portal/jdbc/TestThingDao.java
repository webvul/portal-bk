...
//
//		Optional<List<GlobalThingInfo>> task = globalThingDao.findByIDsAndType(Arrays.asList(thingId1, thingId2).stream().
//				collect(Collectors.toSet()), "LED");
//		assertNotNull("Should have a thing", task.get());
//		assertEquals("Number of things is incorrect", 1, task.get().size());
//		assertEquals("Thing id doesn't match", thingId1, task.get().get(0).getId());
//
//		task = globalThingDao.findByIDsAndType(Arrays.asList(thingId1, thingId2).stream().
//				collect(Collectors.toSet()), "TV");
//		assertNotNull("Should have a thing", task.get());
//		assertEquals("Number of things is incorrect", 1, task.get().size());
//		assertEquals("Thing id doesn't match", thingId2, task.get().get(0).getId());
//
...
//		}
//
//		List<Map<String, Object>> task = globalThingDao.findThingTypesWithThingCount(thingIds).orElseThrow(() ->
//				new RuntimeException("Test fail. Can't get thingTypesWithThingCount"));
//		assertEquals("There should be two objects", 2, task.size());
//		task.forEach(data -> {
...
//		});
//
//		task = globalThingDao.findThingTypesWithThingCount(Collections.emptySet()).orElse(Collections.emptyList());
