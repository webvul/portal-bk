...
public class TestThingUserGroupRelationDao extends TestTemplate {
    ...
    public void testFindThingIds() throws Exception {
        List<Long> thingIds = thingUserGroupRelationDao.findThingIds(this.userGroupIds.get(0));
        assertNotNull("Thing ids should not be null", thingIds);
        assertEquals("There should be two thing ids", 2, thingIds.size());
        assertTrue("The thing ids are incorrect.", new HashSet<Long>(thingIds).containsAll(this.group1ThingIds));

        thingIds = thingUserGroupRelationDao.findThingIds(this.userGroupIds.get(1));
        assertNotNull("Thing ids should not be null", thingIds);
        assertEquals("There should be two thing ids", 2, thingIds.size());
        assertTrue("The thing ids are incorrect.", new HashSet<Long>(thingIds).containsAll(this.group2ThingIds));
    }

    @Test
    public void testFindUserGroupIds() throws Exception {
        List<Long> userGroupIds = thingUserGroupRelationDao.findUserGroupIds(this.allThingIds.get(0));
        assertNotNull("User group ids should not be null", userGroupIds);
        assertEquals("There should be only one user group id", 1, userGroupIds.size());
        assertTrue("The user group id is incorrect.", userGroupIds.get(0).equals(this.userGroupIds.get(0)));

        userGroupIds = thingUserGroupRelationDao.findUserGroupIds(this.allThingIds.get(1));
        assertNotNull("User group ids should not be null", userGroupIds);
        assertEquals("There should be two user group ids", 2, userGroupIds.size());
        ...
        assertNotNull("User group ids should not be null", userGroupIds);
        assertEquals("There should be only one user group id", 1, userGroupIds.size());
        assertTrue("The user group id is incorrect.", userGroupIds.get(0).equals(this.userGroupIds.get(1)));
    }
