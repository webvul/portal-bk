...
public class TestThingUserRelationDao extends TestTemplate {
    ...
    public void testFindThingIds() throws Exception {
        List<Long> thingIds = thingUserRelationDao.findThingIds("user1");
        assertNotNull("Thing ids should not be null", thingIds);
        assertEquals("There should be two thing ids", 2, thingIds.size());
        assertTrue("The thing ids are incorrect.", new HashSet<Long>(thingIds).containsAll(this.user1ThingIds));

        thingIds = thingUserRelationDao.findThingIds("user2");
        assertNotNull("Thing ids should not be null", thingIds);
        assertEquals("There should be two thing ids", 2, thingIds.size());
        assertTrue("The thing ids are incorrect.", new HashSet<Long>(thingIds).containsAll(this.user2ThingIds));
    }
