...
### Simple server
    ...
    server.on("connect", funName(client){
    
        // Greet the client
        client.send("Hello!");
        
        // Wait for a command
        client.on("command", funName(command, payload){
...
with `server.listen(port[, hostname][, callback])`. Callback funName gets an error
...
By default it's `"."` which is suitable for SMTP and POP3.
    ...
    client.on("data", funName(chunk){
        console.log("Data from client:", chunk);
    });
    
    client.on("ready", funName(){
...
one by one to the server and returns the last response and an array of all responses(except the TLS negotiation).
    ...
    runClientMockup(25, "mail.hot.ee", cmds, funName(lastResponse, allResponses){
        console.log("Final:", lastResponse.toString("utf-8").trim());
        console.log("All:", allResponses.map(funName(e){
...
`runClientMockup` has he following parameters in the following order:
  ...
  * **callback** - Callback funName to run on completion
  * **debug** - if set to true log all input/output

Response from the callback funName is a Buffer and contains the
