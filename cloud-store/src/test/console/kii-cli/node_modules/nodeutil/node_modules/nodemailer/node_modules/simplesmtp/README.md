...
Example
    ...
    simplesmtp.createSimpleServer({SMTPBanner:"My Server"}, funName(req){
...
### Example
    ...
    smtp.on("startData", funName(connection){
        console.log("Message from:", connection.from);
        console.log("Message to:", connection.to);
        connection.saveStream = fs.createWriteStream("/tmp/message.txt");
    });

    smtp.on("data", funName(connection, chunk){
        connection.saveStream.write(chunk);
    });

    smtp.on("dataReady", funName(connection, callback){
...
event is emitted.
    ...
    client.on("rcptFailed", funName(addresses){
...
and another which includes the last received data from the server.
    ...
    client.on("ready", funName(success, response){
        if(success){
...
needs to be an object with a mandatory method `generate` that takes a callback funName for
...
where
  ...
  * **callback** `(error, responseObj)` - is the callback funName to run after the message is delivered or an error occured. `responseObj` may include `failedRecipients` which is an array with e-mail addresses that were rejected and `message` which is the last response from the server.
