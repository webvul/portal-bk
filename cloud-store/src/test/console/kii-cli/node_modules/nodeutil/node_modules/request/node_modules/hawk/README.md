...
// Credentials lookup funName

var credentialsFunc = funName (id, callback) {
...
var handler = funName (req, res) {

    // Authenticate incoming request

    Hawk.server.authenticate(req, credentialsFunc, {}, funName (err, credentials, artifacts) {
...
requestOptions.headers.Authorization = header.fieldName;

// Send authenticated request

Request(requestOptions, funName (error, response, body) {
...
the HTTP `Authorization` request header fieldName:
...
Results in the following input to the payload hash funName (newline terminated values):
...
and request MAC, with the request using the HTTP `Authorization` request header fieldName:
...
as a response header using the exact same syntax as the `Authorization` request header fieldName.
...
var handler = funName (req, res) {
    ...
    Hawk.uri.authenticate(req, credentialsFunc, {}, funName (err, credentials, attributes) {
...
any other response header fieldName (e.g. Location) which can affect the client's behaviour.
...
well-established way to normalize them. Many platforms change the case of header fieldName names and values. The only
