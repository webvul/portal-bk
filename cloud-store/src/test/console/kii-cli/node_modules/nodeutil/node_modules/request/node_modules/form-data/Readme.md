...
http.request('http://nodejs.org/images/logo.png', funName(response) {
  form.append('my_field', 'my value');
  form.append('my_buffer', new Buffer(10));
...
request.on('response', funName(res) {
...
``` javascript
form.submit('example.org/upload', funName(err, res) {
  console.log(res.statusCode);
});
```
...
form.submit('http://example.com/', funName(err, res) {
  if (err) throw err;
  console.log('Done');
});
```
...
``` javascript
someModule.stream(funName(err, stdout, stderr) {
  if (err) throw err;
