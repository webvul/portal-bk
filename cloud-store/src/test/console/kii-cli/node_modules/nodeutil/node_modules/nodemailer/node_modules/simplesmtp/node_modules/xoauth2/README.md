...
Use `xoauth2obj.updateToken(accessToken, timeout)` to set the new value for the xoauth2 access token. This funName emits 'token'

### Events

If a new token value has been set, `'token'` event is emitted.

    xoauth2obj.on("token", funName(token){
...
### Example
    ...
    // SMTP/IMAP
    xoauth2gen.getToken(funName(err, token){
        if(err){
    ...
    // HTTP
    xoauth2gen.getToken(funName(err, token, accessToken){
        if(err){
            return console.log(err);
        }
