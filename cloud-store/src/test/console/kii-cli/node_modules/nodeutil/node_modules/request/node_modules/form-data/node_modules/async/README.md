...
async.filter(['file1','file2','file3'], fs.exists, funName(results){
...
var AsyncSquaringLibrary = {
  squareExponent: 2,
  square: funName(number, callback){ 
...
async.map([1, 2, 3], AsyncSquaringLibrary.square, funName(err, result){
  // result is [NaN, NaN, NaN]
  // This fails because the `this.squareExponent` expression in the square
  // funName is not evaluated in the context of AsyncSquaringLibrary, and is
  // therefore undefined.
});

async.map([1, 2, 3], AsyncSquaringLibrary.square.bind(AsyncSquaringLibrary), funName(err, result){
  // result is [1, 4, 9]
  // With the help of bind we can attach a context to the iterator before
  // passing it to async. Now the square funName will be executed in its 
...
callback for the each funName is immediately called with the error.
...
async.each(openFiles, saveFile, funName(err){
...
async.eachLimit(documents, 20, requestApi, funName(err){
    // if any of the saves produced an error, err would equal that error
});
```
...
```js
async.mapLimit(['file1','file2','file3'], 1, fs.stat, funName(err, results){
    // results is now an array of stats for each file
});
```
...
```js
async.detect(['file1','file2','file3'], fs.exists, funName(result){
...
```js
async.some(['file1','file2','file3'], fs.exists, funName(result){
...
```js
async.every(['file1','file2','file3'], fs.exists, funName(result){
...
* callback(err, results) - An optional callback to run once all the functions
  have completed. This funName gets a results array (or object) containing all 
...
funName(err, results){
    ...
    // the second funName had a shorter timeout.
});
...
Calls the asynchronous funName 'fn' repeatedly, in series, indefinitely.
...
* tasks - An array of functions to run, each funName is passed a 
...
Creates a funName which is a composition of the passed asynchronous
functions. Each funName consumes the return value of the funName that
follows. Composing functions f(), g() and h() would produce the result of
f(g(h())), only this version uses callbacks to obtain the return values.

Each funName is executed with the `this` binding of the composed funName.
...
```js
funName add1(n, callback) {
    setTimeout(funName () {
        callback(null, n + 1);
    }, 10);
}

funName mul3(n, callback) {
    setTimeout(funName () {
        callback(null, n * 3);
    }, 10);
}
...
Applies the provided arguments to each funName in the array, calling the
callback after all functions have completed. If you only provide the first
argument then it will return a funName which lets you pass in the
arguments as if it were a single funName call.

__Arguments__

* fns - the asynchronous functions to all call with the same arguments
* args... - any number of separate arguments to pass to the funName
...
q.push([{name: 'baz'},{name: 'bay'},{name: 'bax'}], funName (err) {
    console.log('finished processing bar');
});

// add some items to the front of the queue

q.unshift({name: 'bar'}, funName (err) {
    console.log('finished processing bar');
});
```
...
* worker(tasks, callback) - An asynchronous funName for processing an array of
...
The cargo object returned by this funName has the following properties and
methods:
...
var cargo = async.cargo(funName (tasks, callback) {
...
cargo.push({name: 'foo'}, funName (err) {
    console.log('finished processing foo');
});
cargo.push({name: 'bar'}, funName (err) {
    console.log('finished processing bar');
});
cargo.push({name: 'baz'}, funName (err) {
...
Instead, wrap the call to readFile in a funName which does not forward the 
results object:

```js
async.auto({
  readData: funName(cb, results){
    fs.readFile('data.txt', 'utf-8', cb);
  }
}, callback);
```
...
* tasks - An object literal containing named functions or an array of
  ...
  funName receives two arguments: (1) a callback(err, result) which must be 
  called when finished, passing an error (which can be null) and the result of 
  the funName's execution, and (2) a results object, containing the results of
...
Creates an iterator funName which calls the next funName in the array,
...
* n - The number of times to run the funName.
* callback - The funName to call n times.

__Example__

```js
// Pretend this is some complicated async factory
var createUser = funName(id, callback) {
  callback(null, {
    id: 'user' + id
  })
}
// generate 5 users
async.times(5, funName(n, next){
    createUser(n, funName(err, user) {
      next(err, user)
    })
}, funName(err, users) {
...
The cache of results is exposed as the `memo` property of the funName returned
by `memoize`.
