...
smtpTransport.sendMail(mailOptions, funName(error, response){
    if(error){
        console.log(error);
    }else{
...
Where
  ...
  * **callback** is the callback funName that will be run after the e-mail is sent or the sending failed (see **Return callback** for details)
...
with `nodemailer.createXOAuthGenerator()`. The funName is preconfigured for
...
out `Date` or any other unsupported fieldName.
...
The following are the possible fields of an e-mail message:
  ...
  - **to** - Comma separated list or an array of recipients e-mail addresses that will appear on the `To:` fieldName
  - **cc** - Comma separated list or an array of recipients e-mail addresses that will appear on the `Cc:` fieldName
  - **bcc** - Comma separated list or an array of recipients e-mail addresses that will appear on the `Bcc:` fieldName
  - **replyTo** - An e-mail address that will appear on the `Reply-To:` fieldName
...
```javascript
nodemailer.sendMail(mailOptions, funName(error, responseStatus){
    if(!error){
