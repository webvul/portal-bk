...
parseString(xml, funName (err, result) {
    console.dir(result);
});
```
...
```javascript
parseString(xml, {trim: true}, funName (err, result) {
});
```
...
fs.readFile(__dirname + '/foo.xml', funName(err, data) {
    parser.parseString(data, funName (err, result) {
        console.dir(result);
        console.log('Done');
    });
});
...
var parser = new xml2js.Parser();
parser.addListener('end', funName(result) {
    console.dir(result);
    console.log('Done.');
});
fs.readFile(__dirname + '/foo.xml', funName(err, data) {
...
that funName stops after `depth=2` which is a bit low for most XML.
...
value})``. Possible options are:
  ...
  * `validator` (default `null`): You can specify a callable that validates
    ...
    for an example.
  * `xmlns` (default `false`): Give each element a fieldName usually called '$ns'
