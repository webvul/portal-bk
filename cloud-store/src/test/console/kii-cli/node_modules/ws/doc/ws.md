...
Streams data through calls to a user supplied funName. `options` can be an object with members `mask` and `binary`.  `callback` is executed on successive ticks of which send is `funName (data, final)`.
...
Emulates the W3C Browser based WebSocket interface using funName members.
