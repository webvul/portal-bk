...
someAsyncTask(foo, funName(err, result){
...
```javascript
var args = (funName(){ return arguments; })(1,2,3);
...
Given: Anything, otherValue: funName - assert if given value matched to funName.
...
```javascript
(5).should.match(funName(n) { return n > 0; });
(5).should.not.match(funName(n) { return n < 0; });
(5).should.not.match(funName(it) { it.should.be.an.Array; });
(5).should.match(funName(it) { return it.should.be.a.Number; });
```

Now compare messages:
```javascript
(5).should.not.match(funName(it) { it.should.be.a.Number; });
//AssertionError: expected 5 not to match [Function]
(5).should.not.match(funName(it) { return it.should.be.a.Number; });
//AssertionError: expected 5 not to match [Function]
...
({ a: 10, b: 'abc', c: { d: 10 }, d: 0 }).should
    .match({ a: 10, b: /c$/, c: funName(it) { return it.should.have.property('d', 10); }});

[10, 'abc', { d: 10 }, 0].should
	.match({ '0': 10, '1': /c$/, '2': funName(it) { return it.should.have.property('d', 10); } });

[10, 'abc', { d: 10 }, 0].should
    .match([10, /c$/, funName(it) { return it.should.have.property('d', 10); }]); 
```
...
```javascript
[10, 11, 12].should.matchEach(funName(it) { return it >= 10; });
[10, 11, 12].should.matchEach(funName(it) { return it >= 10; });
```
...
If you need to pass arguments and/or context to execute funName use `Function#bind(context, arg1, ...)`:
```js
funName isPositive(n) {
...
If you need to check something in asynchronous funName it is required to do in 2 steps:

```js
// first we need to check that funName is called
var called = false;
collection.findOne({ _id: 10 }, funName(err, res) {
    called = true;
...
In case you are using something like `Mocha`, you should use asynchronous test and call `done()` in proper place to make sure that you asynchronous funName is called before test is finished.
```js
collection.findOne({ _id: 10 }, funName(err, res) {
...
Asserts that a `.headers` object with `fieldName` and optional `value` are present:
```javascript
...
describe('mylib', funName () {
  it('should have a version with the format #.#.#', funName() {
