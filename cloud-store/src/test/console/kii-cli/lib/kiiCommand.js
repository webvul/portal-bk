...
exports.deployFile = function(program, cmd) {
  ...
  http.login(ctx, function(context) {
    deployFiles(context, cmd.file, cmd.hookConfig, cmd.setCurrent);
  }); 
}
...
function deployFiles(ctx, codeFile, hookConfig, isSetToCurrent) {
  var codeData;
  try {
  ...
  if (configData) {
    ...
    deployCode(ctx, codeData,  function(ctx, versionID) {
    ...
    deployCode(ctx, codeData, (isSetToCurrent ? setCurrent: null));
  }
}

function deployCode(ctx, file, callback) {
...
function listFiles(ctx, utcTime) {
  ...
  http.send(ctx, ctx.versionsPath, 'GET', {}, null, function(responseBody) {
    ...
    if(res.versions.length != 0) {
      ...
      util.puts(sprintf(pattern, 'ID', 'CREATED' + tz, 'STATUS'));
      util.puts(sprintf(pattern, "-", "-", "--------").replace(/\s/gm, '-'));
      // print list of versions
      res.versions.forEach(function(v, i, array) {
        util.puts(sprintf(pattern, 
...
function listExecutions(ctx, from, to, utcTime) {
  ...
  // print header
  var tz =  utcTime ? " (UTC)" : " (LOCAL)";
  util.puts(sprintf(pattern, "ID", "STARTED" + tz, "FINISHED" + tz, "STATUS", "JOB NAME"));
  util.puts(sprintf(pattern, "-", "-", "-", "-", "--------").replace(/\s/gm, '-'));
  // start query
  doQuery(q);
}
